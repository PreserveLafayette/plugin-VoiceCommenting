...
class VoiceCommenting_CommentingController extends Omeka_Controller_AbstractActionController
{
    ...
    {
        $this->_helper->db->setDefaultModelName('VoiceComment');    
    }
    ...
    public function addAction()
    {
        ...
        if(!$valid) {
            $destination .= "#voice-comment-form";
            $commentSession = new Zend_Session_Namespace('voicecommenting');
        ...
        if($requiresApproval) {
            $this->_helper->flashMessenger(__("Your voice comment is awaiting moderation"), 'success');
        }
        ...
        $comment->checkSpam();
        $comment->save();
        $destination .= "#voice-comment-" . $comment->id;
        $this->_helper->redirector->gotoUrl($destination);
    }
    ...
    public function updateFlaggedAction()
    {
        ...
        if($commentIds) {
            foreach($commentIds as $id) {
                $comment = $this->_helper->db->getTable('VoiceComment')->find($id);
    ...
    public function flagAction() {
        $commentId = $_POST['id'];
        $comment = $this->_helper->db->getTable('VoiceComment')->find($commentId);
    ...
    private function getForm()
    {
        ...
        return new VoiceCommenting_CommentForm();
    }
